...
async function initializeFork() {
    try {
        ...
        const settingsDir = path.join(process.env.APPDATA || process.env.HOME || '', 
            'Code', 'User', 'globalStorage', `${publisherId}.${repoName}`, 'settings')
        ...
        const vscodeSettingsPath = path.join(process.env.APPDATA || process.env.HOME || '',
        ...
        const settingsPrefix = repoName as keyof VSCodeSettings
