    ...
    try {
        ...
        for (const file of files) {
            if (file !== 'node_modules' && file !== 'dist' && file !== '.git') {
                await fs.rename(path.join(currentDir, file), path.join(targetDir, file));
            }
        }
        ...
        const settingsDir = path.join(process.env.APPDATA || process.env.HOME || '', 'Code', 'User', 'globalStorage', 'DrugsAreMyLife.nucode', 'settings');
