...
suite("Roo Code Extension Test Suite", () => {
    ...
    test("Views should be registered", () => {
        const view = vscode.window.createWebviewPanel("roo-cline.SidebarProvider", "Roo Code", vscode.ViewColumn.One, {});
        ...
        try {
            ...
            while (Date.now() - startTime < timeout) {
                ...
                if (messages && messages.length > 0) {
                    ...
                    // @ts-ignore
                    const hasResponse = messages.some((m) => m.type === "say" && m.text && m.text.toLowerCase().includes("cline"));
                    if (hasResponse) {
                ...
                if (clineMessages && clineMessages.length > 0) {
                    ...
                    // @ts-ignore
                    const hasResponse = clineMessages.some((m) => m.type === "say" && m.text && m.text.toLowerCase().includes("cline"));
                    if (hasResponse) {
