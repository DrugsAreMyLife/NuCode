...
export async function activate(context: vscode.ExtensionContext) {
	...
	const customModesManager = new CustomModesManager(
		context,
		async () => {
			// Handle mode updates
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(`${CONFIG_PREFIX}.start`, () => {
	...
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider(
			`${CONFIG_PREFIX}.modes`,
			new ModesWebviewProvider(context, customModesManager)
		),
		vscode.window.registerWebviewViewProvider(
			`${CONFIG_PREFIX}.updates`,
			new UpdatesWebviewProvider(context, updateChecker)
		)
	)
...
class ModesWebviewProvider implements vscode.WebviewViewProvider {
	constructor(
		private readonly context: vscode.ExtensionContext,
		private readonly modesManager: CustomModesManager
	) {}
...
class UpdatesWebviewProvider implements vscode.WebviewViewProvider {
	constructor(
		private readonly context: vscode.ExtensionContext,
		private readonly updateChecker: UpdateCheckerServer
	) {}
