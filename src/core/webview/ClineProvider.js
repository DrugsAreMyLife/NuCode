...
export class ClineProvider {
    ...
    */
    ...
    async initClineWithTask(task, images) {
        await this.clearTask();
        const { apiConfiguration, customModePrompts, diffEnabled, fuzzyMatchThreshold, mode, customInstructions: globalInstructions, experiments, } = await this.getState();
        const modePrompt = customModePrompts?.[mode];
        const effectiveInstructions = [globalInstructions, modePrompt?.customInstructions].filter(Boolean).join("\n\n");
        this.cline = new Cline(this, apiConfiguration, effectiveInstructions, diffEnabled, fuzzyMatchThreshold, task, images, undefined, experiments);
    }
    async initClineWithHistoryItem(historyItem) {
        ...
        const modePrompt = customModePrompts?.[mode];
        const effectiveInstructions = [globalInstructions, modePrompt?.customInstructions].filter(Boolean).join("\n\n");
        this.cline = new Cline(this, apiConfiguration, effectiveInstructions, diffEnabled, fuzzyMatchThreshold, undefined, undefined, historyItem, experiments);
    }
        ...
        webview.onDidReceiveMessage(async (message) => {
            switch (message.type) {
                ...
                case "cancelTask":
                    if (this.cline) {
                        ...
                        await pWaitFor(() => this.cline === undefined || this.cline.didFinishAborting, {
                            ...
                            this.outputChannel.appendLine(`Failed to abort task ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`);
                        });
                        if (this.cline) {
                ...
                case "getSystemPrompt":
                    try {
                        const { apiConfiguration, customModePrompts, customInstructions, preferredLanguage, browserViewportSize, diffEnabled, mcpEnabled, fuzzyMatchThreshold, experiments, } = await this.getState();
                        ...
                        const customModes = await this.customModesManager.getCustomModes();
                        const systemPrompt = await SYSTEM_PROMPT(this.context, cwd, apiConfiguration.openRouterModelInfo?.supportsComputerUse ?? false, mcpEnabled ? this.mcpHub : undefined, diffStrategy, browserViewportSize ?? "900x600", mode, customModePrompts, customModes, customInstructions, preferredLanguage, diffEnabled, experiments);
                        await this.postMessageToWebview({
                            type: "systemPrompt",
                            text: systemPrompt,
                            mode: message.mode,
