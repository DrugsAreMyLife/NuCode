...
export class Cline {
    ...
    constructor(provider, apiConfiguration, customInstructions, enableDiff, fuzzyMatchThreshold, task, images, historyItem, experiments) {
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const { browserViewportSize, mode, customModePrompts, preferredLanguage, experiments } = (await this.providerRef.deref()?.getState()) ?? {};
        ...
        const systemPrompt = await (async () => {
            ...
            return SYSTEM_PROMPT(provider.context, cwd, this.api.getModel().info.supportsComputerUse ?? false, mcpHub, this.diffStrategy, browserViewportSize, mode, customModePrompts, customModes, this.customInstructions, preferredLanguage, this.diffEnabled, experiments);
        })();
                        ...
                        try {
                            if (block.partial) {
                                ...
                                const completeMessage = JSON.stringify({
                                    ...
                                    diff: fileExists
                                        ? formatResponse.createPrettyPatch(relPath, this.diffViewProvider.originalContent, newContent)
                                        : undefined,
                                });
                    ...
                    case "switch_mode": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                const targetMode = getModeBySlug(mode_slug, (await this.providerRef.deref()?.getState())?.customModes);
                                if (!targetMode) {
                    ...
                    case "new_task": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                this.consecutiveMistakeCount = 0;
                                // Verify the mode exists
                                const targetMode = getModeBySlug(mode, (await this.providerRef.deref()?.getState())?.customModes);
                                if (!targetMode) {
                                ...
                                if (provider) {
                                    ...
                                    pushToolResult(formatResponse.toolError("Failed to create new task: provider not available"));
                                }
                                break;
                            }
                        }
    ...
    async getEnvironmentDetails(includeFileDetails = false) {
        ...
        */
        ...
        if (!isToolAllowedForMode("write_to_file", currentMode, customModes ?? [], {
            apply_diff: this.diffEnabled,
        }) &&
